    ...
    return (
        <div className="flex-1 overflow-y-auto p-8 space-y-8 max-w-5xl mx-auto">
            <div className="space-y-2">
                <h2 className="text-3xl font-display font-bold text-text-primary">Knowledge Base</h2>
                <p className="text-text-secondary">Query your documents using AI for contextual answers.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-brand-surface border border-brand-border rounded-2xl p-6 shadow-xl space-y-4">
                    <form onSubmit={handleSearch} className="relative">
                        <input
                            ...
                            className="w-full bg-brand-bg border border-brand-border rounded-xl py-3 pl-12 pr-4 text-text-primary focus:outline-none focus:ring-2 focus:ring-brand-accent transition-all"
                        />
                ...
                <div className="bg-brand-surface border border-brand-border rounded-2xl p-6 shadow-xl flex flex-col items-center justify-center space-y-4 text-center">
                    ...
                    <div className="space-y-1">
                        <h3 className="font-bold text-lg text-text-primary">Ingest Documents</h3>
                        <p className="text-xs text-text-secondary opacity-70">Upload PDF, DOCX, or TXT files to expand knowledge</p>
                    </div>
                    <label className="cursor-pointer bg-brand-border hover:bg-brand-border/80 text-text-primary px-6 py-2 rounded-lg text-sm font-bold transition-all disabled:opacity-50">
                    ...
                    {loading ? (
                        ...
                        <div className="space-y-6">
                            <div className="space-y-4">
                                ...
                                <div className="prose prose-invert max-w-none text-text-primary leading-relaxed text-lg">
                                    {answer}
                                </div>
                            </div>
                            ...
                            {sources.length > 0 && (
                                <div className="pt-6 border-t border-brand-border space-y-4">
                                    <h4 className="text-xs font-bold text-text-secondary uppercase tracking-widest">Sources Used</h4>
                                    <div className="flex flex-wrap gap-3">
                                        {sources.map((src, i) => (
                                            <div key={i} className="flex items-center gap-2 bg-brand-bg border border-brand-border px-3 py-1.5 rounded-lg text-xs font-medium text-text-secondary">
                                                <FileText size={14} className="text-brand-accent" />
                                                <span>{src.source.split(/[\\/]/).pop()}</span>
                                                <span className="text-[10px] bg-brand-border px-1.5 rounded text-text-secondary opacity-70">{Math.round(src.top_score * 100)}% Match</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
