  ...
  return (
    <div className="flex-1 overflow-y-auto bg-brand-bg p-4 md:p-8 space-y-8">
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-2">
            <h2 className="text-4xl font-display font-black text-text-primary tracking-tight">Intelligence <span className="text-brand-accent">Hub</span></h2>
            <p className="text-text-secondary font-medium">Real-time performance metrics and quality insights.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="gradient-border bg-brand-surface/30 backdrop-blur-xl p-8 rounded-[2rem] space-y-3 relative overflow-hidden group">
            ...
            <div>
              <p className="text-xs font-black uppercase tracking-[0.2em] text-text-secondary opacity-70">Active Agents</p>
              <p className="text-4xl font-display font-black text-text-primary mt-1">{totalAgents}</p>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.1 }} className="gradient-border bg-brand-surface/30 backdrop-blur-xl p-8 rounded-[2rem] space-y-3 relative overflow-hidden group">
            ...
            <div>
              <p className="text-xs font-black uppercase tracking-[0.2em] text-text-secondary opacity-70">Avg Quality</p>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }} className="gradient-border bg-brand-surface/30 backdrop-blur-xl p-8 rounded-[2rem] space-y-3 relative overflow-hidden group">
            ...
            <div>
              <p className="text-xs font-black uppercase tracking-[0.2em] text-text-secondary opacity-70">Lagging Area</p>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.3 }} className="gradient-border bg-brand-surface/30 backdrop-blur-xl p-8 rounded-[2rem] space-y-3 relative overflow-hidden group">
            ...
            <div>
              <p className="text-xs font-black uppercase tracking-[0.2em] text-text-secondary opacity-70">Total Audits</p>
              <p className="text-4xl font-display font-black text-text-primary mt-1">{audits.length}</p>
            </div>
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 bg-brand-surface/40 backdrop-blur-md border border-brand-border p-8 rounded-[2.5rem] space-y-8 flex flex-col">
            <div className="space-y-1">
              <h3 className="text-xl font-display font-bold text-text-primary">Skills Matrix</h3>
              <p className="text-xs text-text-secondary font-medium">Aggregate metric distribution</p>
            </div>
            ...
            <div className="flex-1 flex items-center justify-center min-h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart cx="50%" cy="50%" outerRadius="80%" data={metricsData}>
                  ...
                  <PolarAngleAxis dataKey="subject" tick={{ fill: 'var(--color-text-secondary)', fontSize: 12, fontWeight: 700 }} />
              ...
              {metricsData.map(m => (
                <div key={m.subject} className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-text-secondary">{m.subject}</span>
                  </div>
                  <span className="text-sm font-black text-text-primary">{m.A}%</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-brand-surface/40 backdrop-blur-md border border-brand-border p-8 rounded-[2.5rem] space-y-6">
              <div className="flex justify-between items-center">
                <div className="space-y-1">
                  <h3 className="text-xl font-display font-bold text-text-primary">Quality & Compliance</h3>
                  <p className="text-xs text-text-secondary font-medium tracking-wide">Performance drift over 30 days</p>
                </div>
                <div className="flex gap-4">
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-1 bg-brand-accent rounded-full"></div>
                    <span className="text-[10px] font-black uppercase text-text-secondary">Quality</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-1 bg-brand-green rounded-full"></div>
                    <span className="text-[10px] font-black uppercase text-text-secondary">Compliance</span>
                  </div>
                </div>
              </div>
              <div className="h-64 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={analytics.trend}>
                    ...
                    <XAxis dataKey="date" stroke="var(--color-text-secondary)" opacity={0.5} fontSize={10} axisLine={false} tickLine={false} />
                    <YAxis domain={[0, 100]} hide />
                    <Tooltip contentStyle={{ backgroundColor: 'var(--color-brand-surface)', border: '1px solid var(--color-brand-border)', borderRadius: '12px', color: 'var(--color-text-primary)' }} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="bg-brand-surface/40 backdrop-blur-md border border-brand-border p-8 rounded-[2.5rem] space-y-6">
                <h3 className="text-lg font-display font-bold text-text-primary">Agent Variance</h3>
              ...
              <div className="bg-brand-surface/40 backdrop-blur-md border border-brand-border p-8 rounded-[2.5rem] space-y-6">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-lg font-display font-bold text-text-primary">Top Performers</h3>
                </div>
                <div className="space-y-4">
                  {leaderboard.map((agent, i) => (
                    <div key={agent.agent_name} className="flex items-center justify-between p-3 rounded-2xl bg-brand-bg/50 border border-brand-border/50">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-brand-bg flex items-center justify-center text-[10px] font-black border border-brand-border text-text-primary">
                          #{i + 1}
                        </div>
                        <span className="text-sm font-bold text-text-primary">{agent.agent_name}</span>
                      </div>
                      <div className="text-right">
                        <p className="text-sm font-black text-brand-green">{agent.avg_score}%</p>
                        <p className="text-[10px] text-text-secondary uppercase font-black">{agent.total_audits} Audits</p>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="bg-gradient-to-r from-brand-accent/20 to-brand-green/10 border border-brand-accent/20 p-10 rounded-[3rem] relative overflow-hidden group">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            <div className="space-y-6">
              ...
              <h3 className="text-3xl font-display font-black text-text-primary dark:text-white leading-tight">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-6 bg-brand-surface/40 backdrop-blur-md border border-brand-border rounded-3xl space-y-2">
                ...
                <p className="text-2xl font-display font-black text-text-primary">94%</p>
                <p className="text-[10px] font-black text-text-secondary uppercase">Compliance Stability</p>
              </div>
              <div className="p-6 bg-brand-surface/40 backdrop-blur-md border border-brand-border rounded-3xl space-y-2">
                <Zap className="text-brand-accent" size={20} />
                <p className="text-2xl font-display font-black text-text-primary">+12%</p>
                <p className="text-[10px] font-black text-text-secondary uppercase">Weekly Velocity</p>
              </div>
            </div>
          </div>
        </div>
